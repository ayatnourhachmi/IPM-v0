...
The frontend renders an `H` / `M` / `L` badge on each tag chip in the Sourcing summary for **Objective**, **Domains**, **Impact**, and **Origin**. The badge color represents confidence, and the chip accent identifies the tag category. Domain chips remain editable/removable from the summary; the other detected fields display their confidence while preserving the existing inline edit controls.

The API also returns a flattened confidence snapshot on analyze and persisted business-need responses:

```json
{
  "confidence": {
    "objectif": "high",
    "domaine": [{ "value": "Finance", "confidence": "high" }],
    "impact": [{ "value": "Cost", "confidence": "high" }],
    "origine": "high"
  }
}
...
| `app/services/catalog_feature_match.py` | Filters catalog hits by need/catalog feature overlap |
| `app/services/embedding_service.py` | Text embedding via local BGE model or OpenAI |
| `app/services/business_impact_scoring.py` | Rules-based need/catalog impact alignment plus catalogue-grounded LLM narration |
| `app/services/duration_formula.py` | Deterministic duration and IVI duration score formula |
| `app/services/expertise_team_service.py` | LLM-based delivery team estimate |
| `app/services/expertise_team_constraints.py` | Allowed expertise role constraints |
...
| Table | Purpose |
|---|---|
| `business_needs` | Core business need records: pitch, horizon, status, JSONB tags, flattened confidence, constraints, duplicate matches, latest risks, justifications, and IVI scores |
...
Tables are created automatically at startup via `Base.metadata.create_all`. Startup also backfills optional JSONB columns (`confidence`, `constraints`, `risks`, `justifications`, `ivi_scores`) when a database was created from an older schema.
...
| `POST` | `/api/v1/needs/expertise-team-estimate` | Estimate required delivery roles from an allowed role list |
| `POST` | `/api/v1/needs/duration-estimate` | Compute deterministic duration and IVI duration score |
| `POST` | `/api/v1/needs/business-impact-score` | Score need/catalog impact alignment and return catalogue-grounded references |
...
The tracked `docker-compose.yml` is safe for both local development and VM deployment. It defaults to local URLs, and the VM overrides come from the machine-local `.env` file, which is not committed.
...
```bash
NEXT_PUBLIC_API_URL=http://20.86.26.141:8000
CORS_ORIGINS=["http://20.86.26.141:3000"]
```
...
| `NEXT_PUBLIC_API_URL` | Frontend API base URL (`http://localhost:8000` locally, VM API URL in deployment) |
| `CORS_ORIGINS` | JSON list of allowed frontend origins for the API |
...
| Sourcing summary panel | `frontend/src/components/sourcing/RecapPanel.tsx` |
| Confidence tag chips component | `frontend/src/components/sourcing/TagChips.tsx` |
