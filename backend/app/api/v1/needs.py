...
from app.services.dxc_context import DXC_CONTEXT, format_dxc_expertise_for_prompt
...
ALLOWED_TRANSITIONS: dict[str, set[str]] = {
    ...
    "submitted": {"solutions_reviewed", "in_qualification", "rework", "abandoned"},
    "solutions_reviewed": {"in_qualification", "rework", "abandoned"},
...
@router.post("/analyze/tags", response_model=AnalyzeTagsResponse)
async def analyze_pitch_tags(request: AnalyzeRequest) -> AnalyzeTagsResponse:
    """Classify a pitch into taxonomy tags (fast path — no reformulation suggestions)."""
    try:
        tags = await nlp_service.tag_pitch(
            request.pitch,
            horizon=request.horizon,
        )
        snap = _confidence_from_tags(tags)
        confidence_layer = TagsConfidence.model_validate(snap) if snap else None
        return AnalyzeTagsResponse(tags=tags, confidence=confidence_layer)
    except Exception as exc:
        logger.error("Failed to tag pitch: %s", exc)
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="LLM tagging failed. Please try again.",
        ) from exc


@router.post("/analyze/suggestions", response_model=AnalyzeSuggestionsResponse)
async def analyze_pitch_suggestions(request: AnalyzeRequest) -> AnalyzeSuggestionsResponse:
    """Generate reformulation suggestions for a pitch (separate from tagging)."""
    try:
        suggestions = await nlp_service.suggest_pitch(
            request.pitch,
            horizon=request.horizon,
        )
        return AnalyzeSuggestionsResponse(suggestions=suggestions)
    except Exception as exc:
        logger.error("Failed to suggest pitch improvements: %s", exc)
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="LLM suggestions failed. Please try again.",
        ) from exc
...
async def gap_analysis(
    ...
    try:
        ...
        if isinstance(ai_scores, dict):
            ...
            # DXC_CONTEXT is empty (sheet missing) fall back to the feature-match proxy.
            _domain_capabilities = DXC_CONTEXT.get(domain, [])
            if not _domain_capabilities:
                # Try case-insensitive match across available domains
                _domain_lower = domain.strip().lower()
                for _ctx_key, _ctx_items in DXC_CONTEXT.items():
                    if _ctx_key.strip().lower() == _domain_lower:
                        _domain_capabilities = _ctx_items
                        break
            if _domain_capabilities:
                # Rich catalog coverage → higher expertise floor; penalise by missing features
                _capability_count = len(_domain_capabilities)
                expertise_eval = _clamp_score_5(
                    min(5, 2 + (_capability_count / 4)) - (missing * 0.15)
                )
                expertise_just = (
                    f"DXC catalog lists {_capability_count} documented capabilities "
                    f"in the '{domain}' domain."
                )
            else:
                # No catalog match — feature-overlap proxy (conservative)
                expertise_eval = _clamp_score_5(1 + matching * 0.4)
                expertise_just = ""
...
async def export_pdf(
    ...
    """Generate the canonical DOCX report and stream it converted to PDF."""
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to convert DOCX report to PDF.",
        ) from exc
