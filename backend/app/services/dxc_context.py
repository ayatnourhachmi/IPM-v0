...
def format_dxc_expertise_for_prompt(context: Dict[str, List[str]], domain: str = "") -> str:
    """Return a compact, prompt-ready summary of DXC capability context.

    If *domain* is supplied, the matching domain section is surfaced first so the
    LLM can anchor the expertise score on the most relevant data.  All other
    domains follow, capped to avoid ballooning the context window.
    """
    if not context:
        return "No structured DXC capability data available."

    domain_key = domain.strip().lower()

    def _section(col: str, items: List[str]) -> str:
        bullet_items = items[:12]  # cap per-domain to keep token count bounded
    ...
    # Surface the matching domain first
    for col, items in context.items():
        if col.strip().lower() == domain_key and items:
            sections.append(_section(col, items))

    # Append remaining domains
    for col, items in context.items():
        if col.strip().lower() != domain_key and items:
            sections.append(_section(col, items))

    return "\n\n".join(sections) if sections else "No structured DXC capability data available."
